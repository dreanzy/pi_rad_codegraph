...
describe("sanitizeDiagnostic", () => {
	...
	it("redacts Bearer tokens", async () => {
		const result = sanitizeDiagnostic(
			"Authorization: Bearer secret-token-value-here",
		);
	...
	it("redacts --api-key, --token, --password flags", async () => {
		const result = sanitizeDiagnostic(
			"--api-key=hidden --token mytoken --otp 123456",
		);
	...
	it("handles API_KEY and APIKEY patterns", async () => {
		expect(sanitizeDiagnostic("API_KEY=supersecret")).toContain(
			"API_KEY=[redacted]",
		);
		expect(sanitizeDiagnostic("APIKEY=supersecret")).toContain(
			"APIKEY=[redacted]",
		);
		expect(sanitizeDiagnostic("MY_AUTH_TOKEN=xyz")).toContain(
...
describe("ensureIndexReady", () => {
	...
	it("auto-inits and runs when .codegraph is missing", async () => {
		...
		const initCalls = mockExecFile.mock.calls.filter(
			(c: any) => c[1][0] === "init",
		);
	...
	it("rebuilds index when sync fails", async () => {
		mockExistsSync.mockReturnValue(true);
		mockExecFile.mockImplementation((_path, args, _opts, cb: Function) => {
			if (args[0] === "sync") return cb(new Error("corrupt index"));
	...
	it("returns NOT_INDEXED_MSG when rebuild also fails", async () => {
		mockExistsSync.mockReturnValue(true);
		mockExecFile.mockImplementation((_path, args, _opts, cb: Function) => {
			if (args[0] === "sync") return cb(new Error("corrupt index"));
			if (args[0] === "index") return cb(new Error("rebuild failed"));
			cb(null, { stdout: "explore result", stderr: "" });
		});

		const result = await execTool("codegraph_explore");
		expect(result.content[0].text).toContain("No .codegraph index");
	});

	it("syncs and passes through when index is healthy", async () => {
		mockExistsSync.mockReturnValue(true);
		mockExecFile.mockImplementation((_path, args, _opts, cb: Function) => {
