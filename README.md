...
CodeGraph tools for [pi](https://pi.dev) Agent — registers `codegraph_explore` and six other CodeGraph callable tools. Auto-initializes the index on first use when `.codegraph` is missing.
...
**Tools are registered whenever the `codegraph` binary is on PATH.** If no `.codegraph` index exists, it's auto-initialized on first tool call. The extension also auto-rebuilds when the index is stale (`reindexRecommended`) or in a partial/failed state, and auto-syncs pending file changes.
